...
char map3[25][51] = {
    ...
    {"10000000000000000000000000001000000000000000000001"},
    {"10000000000000000000000000001000000000000000000001"},
    {"10000000000000000000000000001111111111111111111111"},
    {"10000000000000000000000000000000000000000000000001"},
    {"10000000000000000000034400000000000000000000000001"},
    {"10000000000000000000000000000000000000000000000001"},
    {"10000000000000000000000000000000000000000000000001"},
    {"11111111111111111111111000000000000000000000000001"},
...
void snake::growthItem(){
    ...
    int prevGrowthItemX = growthItemX;
    int prevGrowthItemY = growthItemY;
    ...
    if (growthItemTime >= 100){
        ...
        growthItemTime = 0;
        // item 제거
        if (tmpMap[prevGrowthItemX][prevGrowthItemY] == '3' || tmpMap[prevGrowthItemX][prevGrowthItemY] == '4'){}
        else if (!(prevGrowthItemX==0 && prevGrowthItemY==0)){
            tmpMap[prevGrowthItemX][prevGrowthItemY] = '0';
        }
        while (1){
...
void snake::poisonItem(){
    ...
    int prevPoisonItemX = poisonItemX;
    int prevPoisonItemY = poisonItemY;
    ...
    if (poisonItemTime >= 80){
        ...
        poisonItemTime = 0;
        //item 제거
        if (tmpMap[prevPoisonItemX][prevPoisonItemY] == '3' || tmpMap[prevPoisonItemX][prevPoisonItemY] == '4'){}
        else if (!(prevPoisonItemX==0 && prevPoisonItemY==0)){
            tmpMap[prevPoisonItemX][prevPoisonItemY] = '0';
        }
...
// 게이트 생성
void snake::openGate(){
    int prevGateX1 = gateX1;
    int prevGateY1 = gateY1;
    int prevGateX2 = gateX2;
    int prevGateY2 = gateY2;
    if (gateTime >= 120){
        if (!(prevGateX1==0 && prevGateY1==0)){
            tmpMap[prevGateX1][prevGateY1] = '1';
        }
        if (!(prevGateX2==0 && prevGateY2==0)){
            tmpMap[prevGateX2][prevGateY2] = '1';
        }
        while (1){
            gateX1 = rand()%25;
            gateY1 = rand()%50;
...
bool snake::missionCheck(){
    ...
    if (gItemCnt >= goalGItem && passGateCnt >= goalPassGate && maxLength >= goalMaxLength){
        return true;
    }
    return false;
...
void snake::scoreBoard(){
    ...
    mvprintw(12, 60, "*  B : %d                *", goalMaxLength);
    mvprintw(13, 60, "*  + : %d                *", goalGItem);
    mvprintw(14, 60, "*  - : %d                *", goalPItem);
    mvprintw(15, 60, "*  G : %d                *", goalPassGate);
    mvprintw(16, 60, "*************************");

    if (maxLength >= goalMaxLength){
        mvprintw(12, 82, "V");
    }
    if (gItemCnt >= goalGItem){
        mvprintw(13, 82, "V");
    }
    if (passGateCnt >= goalPassGate){
        mvprintw(15, 82, "V");
    }
...
void snake::gLoop(int mapCode){
    ...
    goalGItem = rand()%4+1;
    goalPItem = rand()%4+1;
    goalPassGate = rand()%3+1;
    goalMaxLength = rand()%3+4;
